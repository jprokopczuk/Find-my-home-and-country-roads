...
def load_data(type):
    ...
    Y_roads_names.sort()
    X = np.array([np.array(Image.open(fname)) for fname in X_names])
    Y = np.array([np.array(Image.open(fname)) for fname in Y_buildings_names])
    Y_r = np.array([np.array(Image.open(fname)) for fname in Y_roads_names])
    X = X.astype('float32')
    Y = Y.astype('float32')
